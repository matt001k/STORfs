...
storfs_err_t crc_compare(storfs_t *storfsInst, storfs_file_header_t storfsInfo,
...
storfs_err_t crc_header_check(storfs_t *storfsInst, storfs_loc_t storfsLoc) {
  ...
  file_header_store_helper(storfsInst, &storfsInfo, storfsLoc,
                           "CRC Header Check");

  while (storfsInfo.fileName[strLen++] != '\0')
    ;

  return crc_compare(storfsInst, storfsInfo, (uint8_t *)storfsInfo.fileName,
                     strLen);
}

storfs_err_t crc_file_check(storfs_t *storfsInst, storfs_loc_t storfsLoc,
                            uint32_t len) {
  storfs_file_header_t storfsInfo;
