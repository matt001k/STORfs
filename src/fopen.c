...
static storfs_err_t fopen_write_flag_helper(storfs_t *storfsInst,
                                            char *pathToFile,
  ...
  if (file_delete_helper(storfsInst, currentOpenFile->fileLoc,
                         currentOpenFile->fileInfo) != STORFS_OK) {
  ...
  while (currentOpenFile->fileInfo.fileName[strLen++] != '\0')
    ;
  currentOpenFile->fileInfo.crc =
      STORFS_CRC_CALC(storfsInst, currentOpenFile->fileInfo.fileName, strLen);
  while (1) {
    if (file_header_create_helper(
            storfsInst, &currentOpenFile->fileInfo, currentOpenFile->fileLoc,
            "Deleting old file and opening new") != STORFS_OK) {
  ...
  if (find_update_next_open_byte(storfsInst, newOpenFile.fileLoc) !=
      STORFS_OK) {
    return STORFS_ERROR;
  }
...
static storfs_err_t validate_mode(const char *mode) {
  ...
  if (!strcmp(mode, "r") || !strcmp(mode, "r+") || !strcmp(mode, "w") ||
      !strcmp(mode, "w+") || !strcmp(mode, "a") || !strcmp(mode, "a+")) {
    return STORFS_OK;
  }
...
storfs_err_t storfs_fopen(storfs_t *storfsInst, char *pathToFile,
  ...
  if (strcmp(mode, "w") == 0) {
    // Determine if the file is already populated with data, and if it is delete
    // the file associated
    if (stream->fileInfo.fileSize > STORFS_HEADER_TOTAL_SIZE) {
      if (fopen_write_flag_helper(storfsInst, pathToFile, stream) !=
          STORFS_OK) {
        goto ERR;
      }
    }
  ...
  STORFS_LOGD(TAG, "File Location: %ld%ld, %ld \r\n \
    File Flags: %ld",
              (uint32_t)(stream->fileLoc.pageLoc >> 32),
              (uint32_t)(stream->fileLoc.pageLoc), stream->fileLoc.byteLoc,
              fileFlags);
