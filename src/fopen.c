...
static storfs_err_t fopen_write_flag_helper(storfs_t    *storfsInst,
  ...
  if(file_delete_helper(storfsInst,
                        currentOpenFile->fileLoc,
  ...
  while(1) {
    if(file_header_create_helper(storfsInst,
                                 &currentOpenFile->fileInfo,
                                 currentOpenFile->fileLoc,
                                 "Deleting old file and opening new") !=
       STORFS_OK) {
...
storfs_err_t storfs_fopen(storfs_t    *storfsInst,
  ...
  if(file_handling_helper(storfsInst,
                          (storfs_name_t *)pathToFile,
                          FILE_OPEN,
                          stream) != STORFS_OK) {
  ...
  STORFS_LOGD(TAG,
              "File Location: %ld%ld, %ld \r\n \
    File Flags: %ld",
              (uint32_t)(stream->fileLoc.pageLoc >> 32),
              (uint32_t)(stream->fileLoc.pageLoc),
              stream->fileLoc.byteLoc,
              fileFlags);
