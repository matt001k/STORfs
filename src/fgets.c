...
storfs_err_t storfs_fgets(storfs_t *storfsInst, char *str, int n,
                          STORFS_FILE *stream) {
  if (storfsInst == NULL || stream == NULL ||
  ...
  if (stream->fileFlags == STORFS_FILE_WRITE_FLAG ||
      stream->fileFlags == STORFS_FILE_APPEND_FLAG) {
  ...
  uint32_t headerLen =
      STORFS_HEADER_TOTAL_SIZE; // Fragment or file header length
  storfs_size_t count =
      n; // Storage for total number of bytes to be read from the file
  storfs_loc_t
      recvDataHeaderLoc; // The location of the current file header in memory
  ...
  if (count < (storfs_size_t)stream->fileRead.fileSizeRem) {
    if (count > storfsInst->pageSize) {
      recvDataItr = (count + STORFS_HEADER_TOTAL_SIZE +
                     ((count / storfsInst->pageSize) *
                      STORFS_FRAGMENT_HEADER_TOTAL_SIZE) +
                     storfsInst->pageSize) /
                    storfsInst->pageSize;
    } else {
      recvDataItr = (count + STORFS_HEADER_TOTAL_SIZE + storfsInst->pageSize) /
                    storfsInst->pageSize;
    }
  } else {
  ...
  do {
    STORFS_LOGD(TAG, "Reading File At %ld%ld, %ld",
                (uint32_t)(stream->fileRead.readLocPtr.pageLoc >> 32),
                (uint32_t)(stream->fileRead.readLocPtr.pageLoc),
                stream->fileRead.readLocPtr.byteLoc);

    // If the receive string buffer length is greater than a page size ensure
    // the received data will maximally be the page size
    if ((count + headerLen) >
        (storfsInst->pageSize - stream->fileRead.readLocPtr.byteLoc)) {
      recvDataLen =
          (storfsInst->pageSize - stream->fileRead.readLocPtr.byteLoc);
      count -= recvDataLen;
    } else {
    ...
    if (storfsInst->read(storfsInst, stream->fileRead.readLocPtr.pageLoc,
                         stream->fileRead.readLocPtr.byteLoc, (uint8_t *)str,
                         recvDataLen) != STORFS_OK) {
    ...
    if (recvDataItr > 0) {
      ...
      stream->fileRead.readLocPtr.pageLoc =
          LOCATION_TO_PAGE(currHeaderInfo.fragmentLocation, storfsInst);
      stream->fileRead.readLocPtr.byteLoc = 0;
      file_header_store_helper(storfsInst, &currHeaderInfo,
                               stream->fileRead.readLocPtr, "");
