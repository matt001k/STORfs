...
storfs_err_t storfs_mount(storfs_t *storfsInst, char *partName) {
  ...
  if((storfsInst->firstByteLoc + STORFS_HEADER_TOTAL_SIZE) >
     storfsInst->pageSize) {
    STORFS_LOGE(TAG,
                "The user defined starting byte and header size is larger "
  ...
  if(((firstPartInfo[0].fileInfo & STORFS_INFO_REG_BLOCK_SIGN_EMPTY) == 0x60) ||
     ((firstPartInfo[1].fileInfo & STORFS_INFO_REG_BLOCK_SIGN_EMPTY) == 0x60)) {
    ...
    if(file_header_create_helper(storfsInst,
                                 ...
                                 "Root") != STORFS_OK) {
      STORFS_LOGE(TAG,
                  "The filesystem could not be created at location %ld%ld, %ld",
    ...
    // filename
    if(crc_compare(storfsInst,
                   firstPartInfo[0],
                   firstPartInfo[0].fileName,
                   strLen) != STORFS_OK) {
      return STORFS_ERROR;
    }
