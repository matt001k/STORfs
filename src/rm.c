...
static storfs_err_t
directory_delete_helper(storfs_t            *storfsInst,
                        storfs_loc_t         rmParentLoc,
  ...
  if(rmParentHeader.childLocation != 0x00) {
    ...
    while(1) {
      ...
      if(rmChildHeader.childLocation != 0x00) {
        if(directory_delete_helper(storfsInst, rmChildFileLoc, rmChildHeader) !=
           STORFS_OK) {
          return STORFS_ERROR;
        }
        rmChildFileLoc.pageLoc =
...
storfs_rm(storfs_t *storfsInst, char *pathToFile, STORFS_FILE *stream) {
  ...
  if((rmStream.fileInfo.fileInfo & STORFS_INFO_REG_FILE_TYPE_FILE) ==
    ...
    if(directory_delete_helper(storfsInst,
                               rmStream.fileLoc,
  ...
  file_header_store_helper(storfsInst,
                           &storfsPreviousHeader,
                           rmStream.filePrevLoc,
                           "Previous");
  ...
  if(rmStream.filePrevLoc.pageLoc == storfsInst->firstPageLoc) {
    ...
    if((storfsPreviousHeader.fileInfo & STORFS_INFO_REG_FILE_TYPE_FILE) ==
      ...
      STORFS_LOGD(TAG,
                  ...
                  (uint32_t)(rmStream.filePrevLoc.pageLoc),
                  0);

      if(storfsInst->read(storfsInst,
                          rmStream.fileLoc.pageLoc,
                          STORFS_HEADER_TOTAL_SIZE,
                          siblingBuf,
  ...
  if(storfsInst->cachedInfo.nextOpenByte >=
     BYTEPAGE_TO_LOCATION(rmStream.fileLoc.byteLoc,
                          rmStream.fileLoc.pageLoc,
                          storfsInst)) {
