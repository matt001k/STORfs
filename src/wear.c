...
storfs_err_t find_prev_file_loc(storfs_t *storfsInst,
  ...
  if (file_header_store_helper(storfsInst, &prevFileHeader, storfsItrLoc,
                               "Previous File") != STORFS_OK) {
    return STORFS_ERROR;
  }
  if (prevFileHeader.childLocation ==
          BYTEPAGE_TO_LOCATION(storfsCurrLoc.byteLoc, storfsCurrLoc.pageLoc,
                               storfsInst) ||
  ...
  if (prevFileHeader.childLocation != 0x00) {
    ...
    if (find_prev_file_loc(storfsInst, storfsCurrLoc, storfsNextChildLoc,
                           storfsPrevLoc) != STORFS_OK) {
      return STORFS_ERROR;
    }

    // If the previously found location is the same as the
    if (BYTEPAGE_TO_LOCATION(storfsPrevLoc->byteLoc, storfsPrevLoc->pageLoc,
                             storfsInst) ==
        BYTEPAGE_TO_LOCATION(storfsCurrLoc.byteLoc, storfsCurrLoc.pageLoc,
                             storfsInst)) {
      return STORFS_OK;
  ...
  if (prevFileHeader.siblingLocation != 0x00) {
    ...
    while (prevFileHeader.siblingLocation != 0x00) {
      ...
      if (file_header_store_helper(storfsInst, &prevFileHeader,
                                   storfsNextSibLoc,
                                   "Previous File") != STORFS_OK) {
        return STORFS_ERROR;
      }
...
storfs_err_t wear_level_act(storfs_t *storfsInst, wear_level_t *wearLevelInfo) {
  ...
  file_header_store_helper(storfsInst, &prevWearLevelInfo.storfsInfo,
                           wearLevelInfo->storfsPrevLoc, "Previous File");

  // If the previous file information is a file, the root or a directory... else
  // if it is a fragment
  if ((prevWearLevelInfo.storfsInfo.fileInfo &
       STORFS_INFO_REG_FILE_TYPE_FILE) == STORFS_INFO_REG_FILE_TYPE_FILE ||
      (prevWearLevelInfo.storfsInfo.fileInfo &
       STORFS_INFO_REG_FILE_TYPE_FILE) == STORFS_INFO_REG_FILE_TYPE_ROOT ||
      (prevWearLevelInfo.storfsInfo.fileInfo &
       STORFS_INFO_REG_FILE_TYPE_FILE) == STORFS_INFO_REG_FILE_TYPE_DIRECTORY) {
    ...
    if (find_prev_file_loc(storfsInst, wearLevelInfo->storfsPrevLoc,
                           storfsInst->cachedInfo.rootLocation[0],
                           &prevWearLevelInfo.storfsPrevLoc) != STORFS_OK) {
      STORFS_LOGE(
          TAG, "Error determining the previous file's parent/sibling location");
      return STORFS_ERROR;
    }
    ...
    if (prevWearLevelInfo.storfsInfo.childLocation ==
      ...
      prevWearLevelInfo.storfsInfo.childLocation = BYTEPAGE_TO_LOCATION(
      ...
      prevWearLevelInfo.storfsInfo.siblingLocation = BYTEPAGE_TO_LOCATION(
          wearLevelInfo->storfsCurrLoc->byteLoc,
          wearLevelInfo->storfsCurrLoc->pageLoc, storfsInst);
    }
  } else {
    ...
    if (tempHeader.fragmentLocation ==
      ...
      while (tempHeader.fragmentLocation !=
        ...
        file_header_store_helper(storfsInst, &tempHeader,
  ...
  STORFS_LOGI(
      TAG, "Previous file's, previous file location %ld%ld",
  ...
  if (storfsInst->read(storfsInst, wearLevelInfo->storfsPrevLoc.pageLoc,
...
storfs_err_t write_wear_level_helper(storfs_t *storfsInst,
  ...
  while (1) {
    STORFS_LOGD(TAG, "Writing File At %ld%ld, %ld",
    ...
    for (int i = 0; i < STORFS_WEAR_LEVEL_RETRY_NUM; i++) {
      ...
      if (storfsInst->write(storfsInst, wearLevelInfo->storfsCurrLoc->pageLoc,
                            wearLevelInfo->storfsCurrLoc->byteLoc,
      ...
      if (wearLevelInfo->storfsFlags & STORFS_FILE_INIT_HEADER_WRITE ||
        ...
        if (crc_file_check(storfsInst, *wearLevelInfo->storfsCurrLoc,
      ...
      if (storfsInst->erase(
              storfsInst, wearLevelInfo->storfsCurrLoc->pageLoc) != STORFS_OK) {
    ...
    if (wearLevelInfo->storfsFlags & STORFS_FILE_WRITE_FLAG &&
      ...
      currInfo.fragmentLocation = BYTEPAGE_TO_LOCATION(
          nextFragmentLoc.byteLoc, nextFragmentLoc.pageLoc, storfsInst);
