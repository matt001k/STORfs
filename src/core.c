...
static void uint16_t_to_uint8_t(uint8_t *buf, uint16_t uint16Val,
                                uint32_t *index);
static void uint32_t_to_uint8_t(uint8_t *buf, uint32_t uint32Val,
                                uint32_t *index);
static void uint64_t_to_uint8_t(uint8_t *buf, uint64_t uint64Val,
                                uint32_t *index);
...
storfs_err_t update_root(storfs_t *storfsInst) {
  ...
  file_header_create_helper(
      storfsInst, &storfsInst->cachedInfo.rootHeaderInfo[0],
      storfsInst->cachedInfo.rootLocation[0], "Root Header 1");
  if (storfsInst->erase(storfsInst,
  ...
  file_header_create_helper(
      storfsInst, &storfsInst->cachedInfo.rootHeaderInfo[1],
      storfsInst->cachedInfo.rootLocation[1], "Root Header 2");
...
storfs_err_t find_update_next_open_byte(storfs_t *storfsInst,
  ...
  update_root_next_open_byte(
      storfsInst, BYTEPAGE_TO_LOCATION(0, storfsLoc.pageLoc, storfsInst));
...
void file_info_display_helper(storfs_file_header_t storfsInfo) {
  STORFS_LOGI(TAG, "\t fileInfo %x \r\n \
        ...
        fileSize %lx \r\n \
        crc %x",
              storfsInfo.fileInfo, storfsInfo.fileName,
              (uint32_t)(storfsInfo.childLocation >> 32),
              (uint32_t)storfsInfo.childLocation,
              (uint32_t)(storfsInfo.siblingLocation >> 32),
              (uint32_t)storfsInfo.siblingLocation, storfsInfo.reserved,
              (uint32_t)(storfsInfo.fragmentLocation >> 32),
              (uint32_t)storfsInfo.fragmentLocation, storfsInfo.fileSize,
              storfsInfo.crc);
}
#else
void file_info_display_helper(storfs_file_header_t storfsInfo) { return 0; }
#endif
...
storfs_err_t file_header_create_helper(storfs_t *storfsInst,
  ...
  STORFS_LOGD(TAG, "Writing %s Header at %ld%ld, %ld", string,
              (uint32_t)(storfsLoc.pageLoc >> 32),
              (uint32_t)(storfsLoc.pageLoc), storfsLoc.byteLoc);
  info_to_buf(headerBuf, storfsInfo);
  if (storfsInst->write(storfsInst, storfsLoc.pageLoc, storfsLoc.byteLoc,
                        headerBuf, STORFS_HEADER_TOTAL_SIZE) != STORFS_OK) {
    status = STORFS_ERR_WRITE_FAILED;
    goto FUNEND;
  }
...
storfs_err_t file_header_store_helper(storfs_t *storfsInst,
  ...
  STORFS_LOGD(TAG, "Storing %s Header at %ld%ld, %ld", string,
              (uint32_t)(storfsLoc.pageLoc >> 32),
              (uint32_t)(storfsLoc.pageLoc), storfsLoc.byteLoc);
  if (storfsInst->read(storfsInst, storfsLoc.pageLoc, storfsLoc.byteLoc,
                       headerBuf, STORFS_HEADER_TOTAL_SIZE) != STORFS_OK) {
...
storfs_err_t find_next_open_byte_helper(storfs_t *storfsInst,
  ...
  while (nextHeaderInfo.fragmentLocation != 0xFFFFFFFFFFFFFFFF ||
    ...
    if (file_header_store_helper(storfsInst, &nextHeaderInfo, *storfsLoc,
                                 "Next") != STORFS_OK) {
      return STORFS_ERROR;
    }
  }
...
storfs_err_t file_handling_helper(storfs_t *storfsInst,
                                  ...
                                  file_action_t actionFlag, void *buff) {
  ...
  storfs_loc_t currentLocation =
      storfsInst->cachedInfo.rootLocation[0]; // Location of the current
  ...
  while (1) {
    ...
    do {
      ...
      if (file_header_store_helper(storfsInst, &wearLevelInfo.storfsInfo,
                                   currentLocation, "Directory") != STORFS_OK) {
        return STORFS_ERROR;
      }
      ...
      if (strcmp((const char *)wearLevelInfo.storfsInfo.fileName,
        ...
        // throughout the path
        currentLocation.pageLoc = LOCATION_TO_PAGE(
            wearLevelInfo.storfsInfo.childLocation, storfsInst);
        currentLocation.byteLoc = LOCATION_TO_BYTE(
            wearLevelInfo.storfsInfo.childLocation, storfsInst);
        ...
        currentLocation.pageLoc = LOCATION_TO_PAGE(
            wearLevelInfo.storfsInfo.siblingLocation, storfsInst);
        currentLocation.byteLoc = LOCATION_TO_BYTE(
            wearLevelInfo.storfsInfo.siblingLocation, storfsInst);
      } else {
        STORFS_LOGD(TAG, "Name not matched, and no siblings, creating "
        ...
        wearLevelInfo.storfsInfo.crc = STORFS_CRC_CALC(
            storfsInst, wearLevelInfo.storfsInfo.fileName, (currStr + 1));
...
storfs_err_t file_delete_helper(storfs_t *storfsInst, storfs_loc_t storfsLoc,
                                storfs_file_header_t storfsInfo) {
  ...
  do {
    STORFS_LOGD(TAG, "Deleting File/Fragment At %ld%ld, %ld",
                (uint32_t)(delDataHeaderLoc.pageLoc >> 32),
                (uint32_t)(delDataHeaderLoc.pageLoc), delDataHeaderLoc.byteLoc);
    ...
    if (delDataItr > 0) {
      ...
      if (file_header_store_helper(storfsInst, &currHeaderInfo,
                                   delDataHeaderLoc, "") != STORFS_OK) {
