...
#ifndef STORFS_LOGI
#define STORFS_LOGI(TAG, fmt, ...) printf("| I |" fmt "\n", ##__VA_ARGS__)
#endif
#ifndef STORFS_LOGD
#define STORFS_LOGD(TAG, fmt, ...) printf("| D |" fmt "\n", ##__VA_ARGS__)
#endif
#ifndef STORFS_LOGW
#define STORFS_LOGW(TAG, fmt, ...) printf("| W |" fmt "\n", ##__VA_ARGS__)
#endif
#ifndef STORFS_LOGE
#define STORFS_LOGE(TAG, fmt, ...) printf("| E |" fmt "\n", ##__VA_ARGS__)
#endif
#else
...
#define STORFS_HEADER_TOTAL_SIZE                                               \
  (STORFS_INFO_REG_SIZE + STORFS_CHILD_DIR_REG_SIZE +                          \
   ...
   STORFS_FILE_SIZE + STORFS_CRC_SIZE + STORFS_MAX_FILE_NAME)
#define STORFS_FRAGMENT_HEADER_TOTAL_SIZE                                      \
  (STORFS_INFO_REG_SIZE + STORFS_RESERVED_SIZE + STORFS_FRAGMENT_LOC_SIZE +    \
   STORFS_CRC_SIZE)
...
/** @brief Location struct for the specific page and byte in that page to
 * read/write to/from */
typedef struct {
...
typedef struct storfs {
   ...
   *
   * @attention   If the offset and the size of the data read is more than the
   * page size the data will start reading from the next page
   *
   ...
   *
   * @attention   If the offset and the size of the data written is more than
   * the page size the data will start writing to the next page
   *
   * @param       storfsInst  Instance used for the STORfs
  ...
  /**
   * @brief       Sync Callback
   *              Callback to sync data, ensure that the memory is ready to
  ...
  /**
   * @brief       CRC Callback
   *              Callback used to determine if a CRC function will be defined
   * by the user
   *
   * @param       storfsInst  Instance used for the STORfs
  ...
  /** @brief Location of the first page and byte within that page in memory for
   * the directory to exist within */
  storfs_size_t firstPageLoc;
  storfs_size_t firstByteLoc;

  /** @brief Size of the page/block/sector/section in bytes within the storage
  device typically 512 Bytes This should be the lowest section available to
  write to a device */
  storfs_size_t pageSize;

  /** @brief Number of erasable page/block/sector/section in bytes within the
   * storage device typically 512 Bytes */
 ...
 *              Used to mount the file system in order to correctly read/write
 * files
 *
 * @attention   When first calling storfs_mount, a name must be used for the
 * partition ex: C:
 *
 * @param       storfsInst  Instance used for the STORfs
 ...
 * @attention   The pathToDir must be a full path from the root to the current
 * directory
 *
 * @param       storfsInst  Instance used for the STORfs
 ...
 *
 * @attention   To remove a directory and all of its contents stream must be
 * NULL
 *
 * @param       storfsInst  Instance used for the STORfs
 * @param       pathToFile  Path to the file from the root partition
 ...
 *
 * @attention   After rewinding the file back to it's origin, appending will
 * truncate the file
 *
 * @param       storfsInst  Instance used for the STORfs
