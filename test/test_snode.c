...
/*!
 @brief Perform a full read on the SNode and compare to the write data

 @details Iteratively performs the read across the file in random or fix sized
          chunks and compares each read chunk to the buffer written to the
          file.

 @param inst SNode instance to write to
 @param write_buf pointer to buffer that was written to file
 @param file_size the size of the file, and write_buf, in bytes
 @param read_offset offset to begin performing reads on
 @param random whether the read chunks should be of random size
 @param chunk_read if not random, the chunk size to read in bytes
 */
...
void test_snode_erase(void) {
  ...
  for(uint8_t i = 0; i < page; i++) {
    fake_storfs_read_page_raw(i, page_buf, page_size);
    for(storfs_size_t j = 0; j < page_size; j++) {
      printf("0x%X ", page_buf[j]);
    }
    printf("\n");
  }
  printf("END HERE\n\n\n\n");
...
//                     STORFS_OK);
//   full_read_helper(&inst, write_buf, buf_size, read_offset, false,
//   read_size);
//
//   random_array_free(write_buf);
//   TEST_ASSERT_EQUAL(snode_erase_data(fs, &inst, &buf_size), STORFS_OK);
...
//     uint32_t     data_remain = buf_size - i;
//     uint32_t     data_size   = MIN(data_remain, chunk_size);
//     storfs_err_t err         = snode_read_data(fs, &inst, read_buf,
//     &data_size); storfs_err_t compare     = get_err_compare(data_remain,
//     chunk_size); TEST_ASSERT_EQUAL(err, compare);
...
//
//     storfs_err_t err = snode_write_data(fs, &inst, &write_buf[i],
//     &data_size); storfs_err_t err_compare = get_err_compare(data_remain,
//     chunk_size); TEST_ASSERT_EQUAL(err, err_compare);
...
//     uint32_t data_remain = buf_size - i;
//     uint32_t data_size   = MIN(data_remain, chunk_size);
//     TEST_ASSERT_EQUAL(snode_write_data(fs, &inst_1, &write_buf[i],
//     &data_size),
//                       STORFS_OK);
//     TEST_ASSERT_EQUAL(snode_read_data(fs, &inst_1, read_buf, &data_size),
//                       STORFS_OK);
//     TEST_ASSERT_EQUAL(memcmp(&write_buf[i], read_buf, data_size), 0);
//     TEST_ASSERT_EQUAL(snode_write_data(fs, &inst_2, &write_buf[i],
//     &data_size),
//                       STORFS_OK);
