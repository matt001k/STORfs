...
struct FlashCtx {
  ...
  uint8_t  fail_flag;
  uint32_t fail_count[OP_COUNT];
  uint32_t fail_tracker[OP_COUNT];
};

static struct FlashCtx ctx = { 0 };

static bool has_failed(FlashOperation op) {

  return ctx.fail_flag & (1 << op) &&
         ++ctx.fail_tracker[op] >= ctx.fail_count[op];
}
...
static storfs_err_t storfs_erase(const storfs_t *fs, storfs_page_t page) {
  if(has_failed(ERASE) && ++ctx.fail_tracker[ERASE] >= ctx.fail_count[ERASE]) {
    return STORFS_ERROR;
  }
