...
int main(void) {
  ...
  /*** Single page write and read test (ensure data is deleted from page to
   * write to) ***/
  ...
  LOGI(TAG,
       "The write/read function to a single block has completed successfully");

  /*** Multiple page write and read test (ensure data is deleted from page to
   * write to) ***/
  uint8_t err = 0;
  fs.write(&fs, FIRST_PAGE_LOCATION, 0, loadBuffer, PAGE_SIZE);
  fs.sync(&fs);
  fs.write(&fs, FIRST_PAGE_LOCATION + 1, 0, (loadBuffer + PAGE_SIZE),
           PAGE_SIZE);
  fs.sync(&fs);
  fs.write(&fs, FIRST_PAGE_LOCATION + 2, 0, (loadBuffer + (2 * PAGE_SIZE)),
           PAGE_SIZE);
  fs.sync(&fs);
  fs.write(&fs, FIRST_PAGE_LOCATION + 3, 0, (loadBuffer + (3 * PAGE_SIZE)),
           PAGE_SIZE);
  fs.sync(&fs);
  fs.read(&fs, FIRST_PAGE_LOCATION, 0, recvBuffer, PAGE_SIZE);
  fs.sync(&fs);
  for (int i = 0; i < PAGE_SIZE; i++) {
    if (loadBuffer[i] != recvBuffer[i]) {
      err++;
      LOGI(TAG,
           "The write/read function to multiple blocks (block 1) has failed");
      break;
    }
  }
  ...
  for (int i = 0; i < PAGE_SIZE; i++) {
    if (loadBuffer[i + PAGE_SIZE] != recvBuffer[i]) {
      err++;
      LOGI(TAG,
           "The write/read function to mutliple blocks (block 2) has failed");
      break;
    }
  }
  ...
  for (int i = 0; i < PAGE_SIZE; i++) {
    if (loadBuffer[i + (2 * PAGE_SIZE)] != recvBuffer[i]) {
      err++;
      LOGI(TAG,
           "The write/read function to mutliple blocks (block 3) has failed");
      break;
    }
  }
  ...
  for (int i = 0; i < PAGE_SIZE; i++) {
    if (loadBuffer[i + (3 * PAGE_SIZE)] != recvBuffer[i]) {
      err++;
      LOGI(TAG,
           "The write/read function to mutliple blocks (block 4) has failed");
      break;
    }
  }
  if (err > 0) {
  ...
  LOGI(TAG,
       "The write/read function to multiple blocks has completed successfully");
