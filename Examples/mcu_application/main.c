...
static void display_cache(storfs_t fs) {
  ...
  next open byte: %ld%ld",
       (uint32_t)(fs.cachedInfo.rootLocation[0].pageLoc >> 32),
       (uint32_t)(fs.cachedInfo.rootLocation[0].pageLoc),
       fs.cachedInfo.rootLocation[0].byteLoc,
       (uint32_t)(fs.cachedInfo.rootLocation[1].pageLoc >> 32),
       (uint32_t)(fs.cachedInfo.rootLocation[1].pageLoc),
       fs.cachedInfo.rootLocation[1].byteLoc,
       (uint32_t)(fs.cachedInfo.nextOpenByte >> 32),
       (uint32_t)fs.cachedInfo.nextOpenByte);
}
...
int main(void) {
  ...
  // Test to ensure that the next open byte will remain equivalent when writing
  // to a file further down the storage system
  ...
  char *strTest =
      ...
      "interdum lacus dolor, sit amet aliquet dolor faucibus id. Sed ac lectus "
      "et diam rhoncus iaculis ut sed diam. Sed vel elit id sem sollicitudin "
      "maximus efficitur quis lacus. Pellentesque tristique enim et magna "
      "condimentum viverra. Phasellus erat neque, euismod a sapien vitae, "
      "auctor tempus diam. Sed ut elit erat. Aliquam dignissim tellus vitae "
      "hendrerit interdum. Aliquam convallis diam non nisi mollis, vitae "
      "eleifend sem tincidunt. Pellentesque ultrices in dolor et viverra. "
      "Maecenas nec dui eget ligula pharetra rutrum sit amet sed nunc.Vivamus "
      "aliquam lorem vel est egestas, vitae porttitor libero ultrices. Vivamus "
      "lacinia cursus dolor, quis ornare sem euismod fringilla. Nunc nisl ex, "
      "cursus et ligula quis, fringilla sodales mi. Nulla facilisi. Vestibulum "
  ...
  LOGI(TAG, "fileName %s \r\n \
    ...
    fileSize %lx \r\n \
    crc %x",
       fs.cachedInfo.rootHeaderInfo[0].fileName,
       fs.cachedInfo.rootHeaderInfo[0].fileInfo,
       (uint32_t)(fs.cachedInfo.rootHeaderInfo[0].childLocation >> 32),
       (uint32_t)fs.cachedInfo.rootHeaderInfo[0].childLocation,
       (uint32_t)(fs.cachedInfo.rootHeaderInfo[0].siblingLocation >> 32),
       (uint32_t)fs.cachedInfo.rootHeaderInfo[0].siblingLocation,
       fs.cachedInfo.rootHeaderInfo[0].reserved,
       (uint32_t)(fs.cachedInfo.rootHeaderInfo[0].fragmentLocation >> 32),
       (uint32_t)fs.cachedInfo.rootHeaderInfo[0].fragmentLocation,
       fs.cachedInfo.rootHeaderInfo[0].fileSize,
       fs.cachedInfo.rootHeaderInfo[0].crc);
}
